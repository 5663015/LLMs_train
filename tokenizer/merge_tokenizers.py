...
parser.add_argument('--llama_tokenizer_dir', default=None, type=str, required=True, help='原分词器文件夹')
...
print('原分词器：')
print('长度：', len(llama_tokenizer))
print('all_special_tokens: ', llama_tokenizer.all_special_tokens)
print('all_special_ids: ', llama_tokenizer.all_special_ids)
print('special_tokens_map: ', llama_tokenizer.special_tokens_map)
...
print("Before: ", len(llama_spm_tokens_set))
for i, spm in enumerate(spms):
    ...
    llama_spm_tokens_set=set(p.piece for p in llama_spm.pieces)
print("New model pieces: ", len(llama_spm.pieces))
...
print("Test text:\n",text)
print("Tokenized by LLaMA tokenizer: ", llama_tokenizer.tokenize(text))
